...
<script type="text/javascript" src="HTML/scripts/function-manage.js"></script>
	<script type="text/javascript">
		...
		{
			//传去servlet
			location.href="productQuafy?page="+page;
		}
	</script>
</head>
<body>
		...
		<div class="manage">
			<table class="list">
				...
				<c:forEach items="${li}" var="i" varStatus="a">
					<tr>
						<td class="first w4 c">${i.ep_id}</td>
						<!-- 跳转到前台商品，获取前台商品信息 -->
						<td class="thumb"><img src="HTML/images/product/${i.ep_file_name}" width="50px" height="40px"/>
						<a href="product-view.html" target="_blank">${i.ep_name}</a>
						</td>
						<td class="w1 c"><a href="productQuaID?ep_id=${i.ep_id}">修改</a>
						<a href="javascript:if(confirm('确定要删除吗？'))location='productDel?ep_id=${i.ep_id}'">删除</a>
						</td>
					</tr>
				</c:forEach>
			</table>
			<div class="pager">
				<ul class="clearfix">
					当前: ${page} / ${maxpage} 页
					<a href="productQuafy?page=1">首页</a>
					<a href="productQuafy?page=${page-1}">上一页</a>
					<a href="productQuafy?page=${page+1}">下一页</a>
					<a href="productQuafy?page=${maxpage}">尾页</a>
