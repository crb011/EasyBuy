...
<div id="main" class="wrap">
	...
	<div class="main">
		<h2>修改分类</h2>
		<div class="manage">
			<form action="productClassUpd" method="post">
				<input type="hidden" name="id" value="${li.epc_id}"/>
				<table class="form">
					<tr>
						<td class="field">父分类：</td>
						<td>
							<select name="parentId">
								<option value="0" <c:if test="${li.epc_parent_id eq 0}">
									selected="selected"
								</c:if>>根栏目</option>
									<c:forEach items="${ea}" var="i">
										<c:if test="${i.epc_parent_id eq 0}">
											<option value="${i.epc_id}" <c:if test="${li.epc_parent_id == i.epc_id}">
									selected="selected"
								</c:if>>${i.epc_name}</option>
										</c:if>
									</c:forEach>
							</select>
						</td>
					...
					<tr>
						...
						<td><input type="text" class="text" name="className" value="${li.epc_name}"/></td>
					</tr>
					<tr>
						<td></td>
